...
def print_status_report(process_status: dict, log_status: dict, 
                       dashboard_status: dict, error_status: dict):
    ...
    try:
        while True:
            ...
            if log_status["status"] == "critical":
                send_alert(f"Log file is stale ({log_status['last_update_minutes_ago']} min old)!", "critical")
            elif log_status["status"] == "stale":
                send_alert(f"Log file is getting stale ({log_status['last_update_minutes_ago']} min old)", "warning")
            
            if dashboard_status["status"] != "ok":
                send_alert("Dashboard is not accessible!", "warning")
            
            if error_status.get("error_count", 0) > 0:
                send_alert(f"Found {error_status['error_count']} errors in last {error_status.get('time_window_minutes', 60)} minutes!", "warning")
