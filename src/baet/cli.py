...
def _load_config(config_path: str | None = None):
    """Load BAET settings from config file."""
    if config_path:
...
@click.pass_context
def ingest(ctx, symbols, days, timeframe):
...
@click.pass_context
def backtest(ctx, strategy, start, end, symbols):
...
@click.pass_context
def paper_trade(ctx, duration, dry_run):
...
def status(ctx):
    ...
    click.echo(f"  Telegram: {'configured' if notif.telegram_bot_token.get_secret_value() else 'not configured'}")
...
def validate(ctx):
    ...
    if settings.notifications.enabled:
        if (
            not settings.notifications.telegram_bot_token.get_secret_value()
