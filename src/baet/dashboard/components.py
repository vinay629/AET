...
def render_portfolio_overview(state: dict[str, Any]) -> None:
    ...
    with col1:
        st.metric(label="💵 Cash", value=f"${state.get('cash', 0):,.2f}")

    with col2:
        st.metric(label="📊 Total Value", value=f"${state.get('total_value', 0):,.2f}")
    ...
    with col4:
        ...
        st.metric(label="💼 Positions Value", value=f"${positions_value:,.2f}")
...
def render_recent_trades(trades: list[dict[str, Any]], limit: int = 50) -> None:
    ...
    styled_df = df.style.format({"Cash After": "${:,.2f}", "Value After": "${:,.2f}"})
...
def render_performance_metrics(metrics: dict[str, Any]) -> None:
    ...
    with col5:
        ...
        st.metric(label="Total Return", value=f"{total_ret:.2%}", delta_color=delta_color)

    with col6:
        ann_ret = metrics.get("annualized_return", 0)
        delta_color = "normal" if ann_ret >= 0 else "inverse"
        st.metric(label="Annualized Return", value=f"{ann_ret:.2%}", delta_color=delta_color)
...
def render_brain_transparency(log_entries: list[dict[str, Any]]) -> None:
    ...
    with col2:
        ...
        for name, info in components.items():
            comp_data.append(
                {"Plugin": name, "Score": info.get("score", 0.0), "Weight": info.get("weight", 1.0)}
            )
...
def render_daily_summary(summary: dict[str, Any]) -> None:
    ...
    with col4:
        ...
        st.metric(label="🔍 Risk Evaluations", value=risk_evals, delta=f"{risk_evals} checks run")
...
def render_log_viewer(entries: list[dict[str, Any]], max_entries: int = 100) -> None:
    ...
    selected_types = st.multiselect("Filter by type:", options=entry_types, default=entry_types)
