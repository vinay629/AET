...
def render_control_panel():
    ...
    with col2:
        if st.button("⏸️ Stop", disabled=not status.get("running"), use_container_width=True):
    ...
    with st.sidebar.expander("⚙️ Settings"):
        ...
        if config:
            ...
            new_duration = st.selectbox(
                ...
                index=0
                if config.get("duration_days") is None
                else [1, 7, 30].index(config.get("duration_days")) + 1
                if config.get("duration_days") in [1, 7, 30]
                else 0,
            )
...
def render_status_bar():
    ...
    with col3:
        if status.get("running") and st.button("🚨 Emergency Stop", type="primary"):
            success, msg = emergency_stop()
            if success:
                st.error(msg)
                st.rerun()
