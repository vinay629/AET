...
class RiskEngine:
    ...
    def evaluate_signal(
        ...
        return RiskCheckResult.approve(
            action=signal.get("action", "HOLD"), size=adjusted_size, reasons=reasons
        )

    def _check_position_sizing(self, signal: dict, current_size: float) -> RiskCheckResult:
        ...
        if adjusted_size < self.policy.position_sizing.min_position_size:
            return RiskCheckResult.reject(
                action=signal.get("action", "HOLD"),
                reasons=[
                    f"Position size {current_size} below minimum {self.policy.position_sizing.min_position_size}"
                ],
                violations=[
                    RiskViolation(
        ...
        max_risk_amount = (
            self.policy.position_sizing.max_risk_per_trade * self.portfolio_state["total_equity"]
        )
        ...
        return RiskCheckResult.approve(
            action=signal.get("action", "HOLD"), size=adjusted_size, reasons=["Position sizing OK"]
        )
    ...
    def _check_drawdown(self, signal: dict) -> RiskCheckResult:
        ...
        if peak_equity <= 0:
            return RiskCheckResult.approve(
                action=signal.get("action", "HOLD"), reasons=["No drawdown data yet"]
            )
        ...
        if current_drawdown < -self.policy.drawdown.kill_switch_drawdown:
            ...
            return RiskCheckResult.reject(
                action=signal.get("action", "HOLD"),
                reasons=[
                    f"Kill-switch triggered: drawdown {current_drawdown:.2%} exceeds {self.policy.drawdown.kill_switch_drawdown:.2%}"
                ],
                violations=[
                    RiskViolation(
            ...
            return RiskCheckResult.reject(
                ...
                violations=[
                    RiskViolation(
                        ...
                        threshold_value=-self.policy.drawdown.max_daily_loss
        ...
        return RiskCheckResult.approve(
            action=signal.get("action", "HOLD"), reasons=["Drawdown within limits"]
        )
    ...
    def _check_regime_risk(self, signal: dict, regime: RegimeLabel) -> RiskCheckResult:
        ...
        current_exposure = (
            self.portfolio_state["total_exposure"] / self.portfolio_state["total_equity"]
        )
        ...
        if current_exposure >= max_exposure:
            return RiskCheckResult.reject(
                action=signal.get("action", "HOLD"),
                reasons=[
                    f"Exposure {current_exposure:.2%} exceeds regime limit {max_exposure:.2%} for {regime_str}"
                ],
                violations=[
                    RiskViolation(
        ...
        return RiskCheckResult.approve(
            action=signal.get("action", "HOLD"), reasons=[f"Regime {regime_str}: exposure OK"]
        )
    ...
    def _check_strategy_risk(self, signal: dict) -> RiskCheckResult:
        ...
        return RiskCheckResult.approve(
            action=signal.get("action", "HOLD"), reasons=[f"Strategy {strategy_name} approved"]
        )
    ...
    def _check_portfolio_risk(self, signal: dict) -> RiskCheckResult:
        ...
        if (
            ...
            >= self.policy.portfolio.max_total_exposure * self.portfolio_state["total_equity"]
        ):
            return RiskCheckResult.reject(
                action=signal.get("action", "HOLD"),
        ...
        if symbol and symbol in self.portfolio_state["positions"]:
            ...
            if (
                symbol_exposure
                >= self.policy.portfolio.max_concentration * self.portfolio_state["total_equity"]
            ):
                return RiskCheckResult.reject(
                    action=signal.get("action", "HOLD"),
                    ...
                    violations=[
                        RiskViolation(
                            ...
                            threshold_value=self.policy.portfolio.max_concentration
                            * self.portfolio_state["total_equity"],
                            symbol=symbol,
                        )
                    ],
        ...
        if len(self.portfolio_state["positions"]) >= self.policy.portfolio.max_active_positions:
            if signal.get("action") == "BUY" and symbol not in self.portfolio_state["positions"]:
                return RiskCheckResult.reject(
                    action=signal.get("action", "HOLD"),
                    reasons=[
                        f"Max active positions ({self.policy.portfolio.max_active_positions}) reached"
                    ],
                    violations=[
                        RiskViolation(
        ...
        return RiskCheckResult.approve(
            action=signal.get("action", "HOLD"), reasons=["Portfolio risk OK"]
        )
    ...
    def _check_emergency_conditions(self) -> RiskCheckResult:
        ...
        if not self.policy.emergency.kill_switch_enabled:
            return RiskCheckResult.approve(action="HOLD", reasons=["Kill-switch disabled"])

        # Check if in cooldown period
        if self.kill_switch_active and self.kill_switch_time:
            cooldown_end = self.kill_switch_time + timedelta(
                hours=self.policy.emergency.cooldown_period_hours
            )
        ...
        return RiskCheckResult.approve(action="HOLD", reasons=["No emergency conditions"])
