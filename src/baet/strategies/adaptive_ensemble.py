...
class PerformanceTracker:
    ...
    def update(self, strategy_name: str, signals: pd.DataFrame, returns: pd.DataFrame):
        ...
        if strategy_name not in self.metrics:
            self.metrics[strategy_name] = pd.DataFrame(
                columns=["timestamp", "return", "cumulative_return"]
            )

        # Calculate strategy returns (simplified: signal * next period return)
        merged = pd.merge(
            signals[["timestamp", "action", "target_position"]],
        ...
        new_data = merged[["timestamp", "strategy_return"]].rename(
...
class AdaptiveEnsemble(StaticEnsemble):
    ...
    def update_weights(
        ...
        self.historical_weights.append({"timestamp": pd.Timestamp.now(), "weights": weights.copy()})
