    ...
    try:
        ...
        if account_info["success"]:
            ...
            for asset, data in account_info["balances"].items():
                print(f"     {asset}: {data['free']:.6f} (free) + {data['locked']:.6f} (locked) = {data['total']:.6f}")
        else:
        ...
        if validation["valid"]:
            ...
            print(f"   Signal: {validation['signal']['side']} {validation['signal']['quantity']} {validation['signal']['symbol']}")
