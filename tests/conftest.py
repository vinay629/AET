...
def pytest_collection_modifyitems(items):
    ...
    - Files with 'integration' in name ΓåÆ integration marker
    - Files with 'e2e' in name ΓåÆ e2e marker
    - Everything else ΓåÆ unit marker (unless already marked)
    """
    for item in items:
...
def pytest_terminal_summary(terminalreporter):
    ...
    if stats:
        ...
        terminalreporter.write_line(
            f"Γ£ô Passed: {total_passed} | Γ£ù Failed: {total_failed} | Γèÿ Skipped: {total_skipped}\n"
        )
