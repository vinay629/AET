...
def test_ensemble_config_creation():
    ...
    config = EnsembleConfig(name="test_ensemble", strategy_weights=weights, default_weight=0.5)
...
def test_static_ensemble_with_regime():
    ...
    regime_weights = {RegimeLabel.TRENDING: {"strategy_a": 2.0, "strategy_b": 0.5}}

    config = EnsembleConfig(
        name="regime_ensemble", strategy_weights=weights, regime_weights=regime_weights
    )
    ensemble = StaticEnsemble(config)
...
def test_static_ensemble_end_to_end():
    ...
    config = EnsembleConfig(
        name="adaptive_ensemble", strategy_weights=weights, regime_weights=regime_weights
    )
    ensemble = StaticEnsemble(config)
    ...
    regime_data = pd.DataFrame(
        {"timestamp": dates, "regime": [RegimeLabel.TRENDING] * 10 + [RegimeLabel.RANGING] * 10}
    )
