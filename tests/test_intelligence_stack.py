...
def create_sample_metrics(
    strategy_names: list, category: str = "baseline", sharpe_range: tuple = (0.5, 1.5)
) -> pd.DataFrame:
        ...
        records.append(
            {
                ...
                "sharpe_ratio": sharpe_range[0]
                + (sharpe_range[1] - sharpe_range[0]) * i / len(strategy_names),
...
def test_build_intelligence_stack_comparison():
    ...
    ensemble = create_sample_metrics(
        ["static_ensemble", "adaptive_ensemble"], "ensemble", (0.8, 1.3)
    )
...
def test_intelligence_stack_end_to_end():
    ...
    # Validate stack
    validation = validate_intelligence_stack(
        baseline, ensemble, ml, min_improvement=10.0
    )  # 10% improvement
