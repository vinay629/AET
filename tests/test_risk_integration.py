...
def test_evaluate_strategy_signal_rejects_blacklisted():
    ...
    policy = RiskPolicy(
        strategy=StrategyRiskPolicy(blacklisted_strategies=["bad_strategy"])
    )
...
def test_evaluate_combined_signals_filters_rejected():
    ...
    policy = RiskPolicy(
        strategy=StrategyRiskPolicy(blacklisted_strategies=["bad"])
    )
    engine = RiskEngine(policy)
...
def test_evaluate_combined_signals_empty():
    ...
    empty_df = pd.DataFrame(columns=['action', 'target_position', 'confidence', 
                                  'size_hint', 'strategy_name', 'symbol', 'timestamp'])
...
def test_evaluate_combined_signals_all_rejected():
    ...
    policy = RiskPolicy(
        strategy=StrategyRiskPolicy(blacklisted_strategies=["all"])
    )
    engine = RiskEngine(policy)
